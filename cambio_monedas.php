...
function myplugin_form_cambio_moneda() {
	?>
	...
	<table border="1">
		...
		<td><input id="montoACambiarTxt" type="text" name="montoACambiarTxt"></td>
		<td>
		<select name="tipoCambioHastaSelect">
		   <option value="AED" title="United Arab Emirates Dirham">AED</option>
		   <option value="AFN" title="Afghan Afghani">AFN</option>
		   <option value="ALL" title="Albanian Lek">ALL</option>
		   <option value="AMD" title="Armenian Dram">AMD</option>
...
function myplugin_process_cambio_moneda() {

	if ( isset( $_POST['tipoCambioDesdeSelect'] ) ) {
		$tipoCambioDesde = sanitize_text_field( $_POST[ 'tipoCambioDesdeSelect' ] );
		$tipoCambioHasta = sanitize_text_field( $_POST[ 'tipoCambioHastaSelect' ] );
		$montoCambio = sanitize_text_field( $_POST[ 'montoACambiarTxt' ] );
		$esValido = true;
		$mensajeValidacion ="";
		
		if ($tipoCambioDesde == $tipoCambioHasta)
		{
			$mensajeValidacion = "No se puede convertir usando la misma moneda</br>";
			$esValido = false;
		}
	
		if (!is_numeric($montoCambio))
		{
			$mensajeValidacion = "El valor introducido no es numérico </br>";
			$esValido = false;
		}
		...
		if ($esValido)
		{
			...
			$tiposDeCambioResponse = json_decode($json, true);
			
			$tipoCambio = $tiposDeCambioResponse[$tipoCambioDesde."_".$tipoCambioHasta];
			$resultado = $montoCambio * $tipoCambio;
			echo "<br>".$montoCambio." ".$tipoCambioDesde."Corresponde a:".$resultado ." ".$tipoCambioHasta ;
		}
		else
		{
...
function cambio_moneda_add_toplevel_menu() {
	...
	add_menu_page(
		...
		'manage_options',
		'Cambio-de-Monedas',
		'cambio_moneda_display_settings_page',
...
function cambio_moneda_display_settings_page() {
	...
	<div class="wrap">
		...
		<?php myplugin_form_cambio_moneda(); ?>
		<?php myplugin_process_cambio_moneda(); ?>
